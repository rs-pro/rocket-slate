...
class CheckListItem extends React.Component {
  ...
  setChecked(checked) {
    ...
    if (editor.props.kind == "issue") {
      axios.patch(`/api/v1/issues/${editor.props.issue.id}/content`, diff).then((r) => {
        //console.log(r.data)
      }).catch((e) => {
        console.error(e)
      ...
      axios.patch(`/api/v1/issues/${editor.props.issue.id}/comments/${editor.props.comment.id}/content`, diff).then((r) => {
        //console.log(r.data)
      }).catch((e) => {
        console.error(e)
        alert("При сохранении чекбокса произошла ошибка")
  ...
  render() {
    ...
    return (
      <span className={cl} {...attributes}>
        ...
        <span
          className={classNames("check-list-item__content-wrapper", {"check-list-item__content-wrapper--checked": checked})}
          checked={checked}
